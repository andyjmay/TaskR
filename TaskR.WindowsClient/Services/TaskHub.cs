...
using TaskR.WindowsClient.Properties;
...
namespace TaskR.WindowsClient.Services {
  public class TaskHub : ITaskHub {
    ...
    public TaskHub() {
      ...
      hubProxy.On<Task>("AddedTask", task => DispatcherHelper.UIDispatcher.BeginInvoke(new Action(() => Messenger.Default.Send(new AddedTaskEvent(task)))));
      hubProxy.On<Task>("UpdatedTask", task => DispatcherHelper.UIDispatcher.BeginInvoke(new Action(() => Messenger.Default.Send(new UpdatedTaskEvent(task)))));
      hubProxy.On<Task>("DeletedTask", task => DispatcherHelper.UIDispatcher.BeginInvoke(new Action(() => Messenger.Default.Send(new DeletedTaskEvent(task)))));
      hubProxy.On<Exception>("HandleException", exception => DispatcherHelper.UIDispatcher.BeginInvoke(new Action(() => Messenger.Default.Send(new ExceptionEncounteredEvent(exception)))));
